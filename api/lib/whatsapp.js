...
export function sendApplicationAccepted(to, requestNumber) {
...
export function sendMainMenu(to) {
  return sendMessage(to, { type: "interactive", interactive: { type: "button", body: { text: "Оберіть, що потрібно зробити:" }, action: { buttons: [
...
export function sendApplicantTypeMenu(to) {
  return sendMessage(to, { type: "interactive", interactive: { type: "button", body: { text: "Хто подає запит?" }, action: { buttons: [
...
export function sendReturningApplicantMenu(to, lastDocumentsUpdatedAt) {
  ...
  return sendMessage(to, { type: "interactive", interactive: { type: "button", body: { text: `Для цього номера вже є збережені дані.\n\nДата останнього оновлення документів: ${date}\n\nВикористати збережені дані чи ввести їх заново?` }, action: { buttons: [
...
export function sendEditMenu(to, type) {
  const rows = type === "military_unit"
    ? [
        { id: "edit_name", title: "ПІБ" },
        { id: "edit_unit_number", title: "Номер ВЧ" },
  ...
  return sendMessage(to, { type: "interactive", interactive: { type: "list", body: { text: "Оберіть, що саме бажаєте змінити:" }, action: {
    button: "Обрати поле",
    sections: [{ title: "Дані заявки", rows }],
  } } });
}

export function sendOptionalDocumentPrompt(to, documentLabel) {
  return sendMessage(to, { type: "interactive", interactive: { type: "button", body: { text: `Будь ласка, надішліть ${documentLabel}.\n\nЦей документ не є обов'язковим. Якщо у вас його немає або ви не хочете додавати його до заявки, натисніть «Пропустити».\n\nЯкщо додаєте документ, можна надіслати один або кілька файлів/фото. Після завантаження натисніть «Готово».` }, action: { buttons: [
...
export async function sendDocumentDone(to, documentLabel, allowSkip = false) {
  ...
  return sendMessage(to, { type: "interactive", interactive: { type: "button", body: { text: `${documentLabel}\n\nФайл отримано. Якщо ви додали всі сторінки/файли цього документа, натисніть «Готово».` }, action: { buttons } } });
}

export function sendYesNoMenu(to, body, yesId = "yes", noId = "no") {
  return sendMessage(to, { type: "interactive", interactive: { type: "button", body: { text: body }, action: { buttons: [

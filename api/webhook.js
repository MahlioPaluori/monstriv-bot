...
import { getOrCreateRequestFolder, uploadBufferToDrive } from "./lib/google-drive.js";
import { appendApplicationToSheet } from "./lib/google-sheets.js";
import {
...
function mediaId(message) { if (message.type === "image") return message.image?.id || null; if (message.type === "document") return message.document?.id || null; return null; }
function documentsAreOlderThanYear(dateString) { if (!dateString) return true; const updated = new Date(dateString).getTime(); if (!Number.isFinite(updated)) return true; return Date.now() - updated > 365 * 24 * 60 * 60 * 1000; }
function fileExtension(message, mimeType) { if (message.type === "document" && message.document?.filename) { const match = message.document.filename.match(/\.([a-z0-9]+)$/i); if (match) return `.${match[1].toLowerCase()}`; } return { "image/jpeg": ".jpg", "image/png": ".png", "image/webp": ".webp", "application/pdf": ".pdf" }[mimeType] || ".bin"; }

async function saveIncomingDocument(from, state, message, fileId, doc) {
  const claimed = await claimMediaUpload(from, fileId); if (!claimed) return false;
  try {
    if (!state.request.data.name) throw new Error("Responsible person name is missing");
    if (state.request.type === "military_unit" && !state.request.data.militaryUnitNumber) throw new Error("Military unit number is missing");
    if (!state.request.applicationNumber) state.request.applicationNumber = await createApplicationNumber();
    const folder = await getOrCreateRequestFolder(state.request);
    ...
    await saveUserState(from, state); return true;
  } catch (error) { await releaseMediaUploadClaim(from, fileId); throw error; }
}
...
function summary(state) {
  ...
  const lines = ["Перевірте, будь ласка, дані заявки:", "", `Номер заявки: ${r.applicationNumber || "буде присвоєно"}`, `Тип заявника: ${r.type === "individual" ? "Фізична особа" : "Військова частина"}`, `ПІБ: ${d.name || "—"}`, `Телефон: ${d.phone || state.phone || "—"}`];
  if (r.type === "military_unit") lines.push(`Номер військової частини: ${d.militaryUnitNumber || "—"}`);
  lines.push(`Місто: ${d.city || "—"}`, `Відділення Нової пошти: ${d.novaPoshtaBranch || "—"}`);
  if (d.recipientAnotherPerson) lines.push(`Отримувач: ${d.recipientName || "—"}`, `Телефон отримувача: ${d.recipientPhone || "—"}`);
  else lines.push("Отримувач: заявник", `Телефон отримувача: ${d.phone || state.phone || "—"}`);
  lines.push(`Документів отримано: ${count}`, "", `Потреба: ${d.need || "—"}`, "", "Якщо все правильно, підтвердіть заявку."); return lines.join("\n");
}
async function showConfirmation(from, state) { if (!state.request.applicationNumber) state.request.applicationNumber = await createApplicationNumber(); state.stage = "CONFIRMATION"; await saveUserState(from, state); await sendText(from, summary(state)); await sendYesNoMenu(from, "Підтвердити заявку?", "confirm_request", "edit_request"); }
async function continueAfterSavedData(from, state) { if (!state.request.documents || !Object.keys(state.request.documents).length) return showConfirmation(from, state); if (documentsAreOlderThanYear(state.request.savedDocumentsUpdatedAt)) { state.stage = "DOCUMENT_UPDATE_DECISION"; await saveUserState(from, state); return sendYesNoMenu(from, "Ваші документи були оновлені понад рік тому. Бажаєте завантажити актуальні документи?", "update_documents_yes", "update_documents_no"); } await showConfirmation(from, state); }
async function askDocument(from, state) { const doc = currentDoc(state); if (!doc) { if (state.request.usingSavedData) return showConfirmation(from, state); state.stage = "CITY"; await saveUserState(from, state); return sendText(from, "Вкажіть, будь ласка, місто, до якого потрібно оформити отримання."); } if (doc.optional) return sendOptionalDocumentPrompt(from, doc.label); await sendText(from, `Будь ласка, надішліть ${doc.label}.\n\nМожна надіслати один або кілька файлів/фото цього документа. Після того як усе надіслано, натисніть «Готово».`); }
async function recipientChoice(from, state, another) { state.request.data.recipientAnotherPerson = another; if (another) { state.stage = "RECIPIENT_NAME"; await saveUserState(from, state); return sendText(from, "Вкажіть, будь ласка, ПІБ іншого отримувача."); } await showConfirmation(from, state); }

export default async function handler(req, res) {
  if (req.method === "GET") { const mode = req.query["hub.mode"], token = req.query["hub.verify_token"], challenge = req.query["hub.challenge"]; if (mode === "subscribe" && token === process.env.WEBHOOK_VERIFY_TOKEN) return res.status(200).send(challenge); return res.status(403).send("Forbidden"); }
  if (req.method !== "POST") return res.status(405).send("Method Not Allowed");
  try {
    const value = req.body?.entry?.[0]?.changes?.[0]?.value, message = value?.messages?.[0]; if (!message) return res.status(200).send("EVENT_RECEIVED");
    ...
    if (["/reset", "reset"].includes(text.toLowerCase())) { await resetUserState(from); await sendText(from, "Тестову сесію скинуто. Починаємо заново."); await sendMainMenu(from); return res.status(200).send("EVENT_RECEIVED"); }
    ...
    if (state.stage === "APPLICANT_TYPE") { if (buttonId === "individual" || buttonId === "military_unit") { state.request.type = buttonId === "individual" ? "individual" : "military_unit"; if (buttonId === "individual") { const profile = await getApplicantProfile(from); if (profile) { state.request.profile = profile; state.stage = "RETURNING_CHOICE"; await saveUserState(from, state); await sendReturningApplicantMenu(from, profile.lastDocumentsUpdatedAt); return res.status(200).send("EVENT_RECEIVED"); } } state.stage = "IDENTIFICATION"; await saveUserState(from, state); await sendText(from, buttonId === "individual" ? "Для ідентифікації заявника вкажіть, будь ласка, ваше ПІБ." : "Для ідентифікації заявника вкажіть, будь ласка, ПІБ відповідальної особи."); } else await sendApplicantTypeMenu(from); return res.status(200).send("EVENT_RECEIVED"); }
    if (state.stage === "RETURNING_CHOICE") { const profile = state.request.profile; if (!profile) { state.stage = "IDENTIFICATION"; await saveUserState(from, state); await sendText(from, "Для ідентифікації заявника вкажіть, будь ласка, ваше ПІБ."); return res.status(200).send("EVENT_RECEIVED"); } if (buttonId === "use_saved_data") { state.request.type = "individual"; state.request.data = { ...(profile.data || {}), phone: from }; state.request.documents = JSON.parse(JSON.stringify(profile.documents || {})); state.request.documentIndex = 0; state.request.savedDocumentsUpdatedAt = profile.lastDocumentsUpdatedAt; state.request.usingSavedData = true; state.stage = "REQUEST_TYPE"; await saveUserState(from, state); await sendText(from, "Збережені дані використано. Тепер напишіть, будь ласка, який саме запит ви хочете подати."); return res.status(200).send("EVENT_RECEIVED"); } if (buttonId === "enter_data_again") { state.request = { type: "individual", data: { phone: from }, documents: {}, documentIndex: 0 }; state.stage = "IDENTIFICATION"; await saveUserState(from, state); await sendText(from, "Введіть, будь ласка, ПІБ заново."); return res.status(200).send("EVENT_RECEIVED"); } await sendReturningApplicantMenu(from, profile.lastDocumentsUpdatedAt); return res.status(200).send("EVENT_RECEIVED"); }
    if (state.stage === "IDENTIFICATION" && text) { state.request.data.name = text; state.stage = state.request.type === "military_unit" ? "MILITARY_UNIT_NUMBER" : "REQUEST_TYPE"; await saveUserState(from, state); await sendText(from, state.request.type === "military_unit" ? "Вкажіть, будь ласка, номер військової частини." : "Дякую. Тепер напишіть, будь ласка, який саме запит ви хочете подати."); return res.status(200).send("EVENT_RECEIVED"); }
    ...
    if (state.stage === "DOCUMENTS") { const doc = currentDoc(state); if (!doc) { await askDocument(from, state); return res.status(200).send("EVENT_RECEIVED"); } if (fileId) { const uploaded = await saveIncomingDocument(from, state, message, fileId, doc); if (!uploaded) return res.status(200).send("EVENT_RECEIVED"); await sendDocumentDone(from, doc.label, Boolean(doc.optional)); return res.status(200).send("EVENT_RECEIVED"); } if (buttonId === "document_done") { const files = state.request.documents[doc.key] || []; if (!files.length) { await sendText(from, `Будь ласка, спочатку надішліть ${doc.label}.`); return res.status(200).send("EVENT_RECEIVED"); } state.request.documentIndex += 1; await saveUserState(from, state); await askDocument(from, state); return res.status(200).send("EVENT_RECEIVED"); } if (buttonId === "document_skip" && doc.optional) { state.request.documentIndex += 1; await saveUserState(from, state); await askDocument(from, state); return res.status(200).send("EVENT_RECEIVED"); } }
    ...
    if (state.stage === "EDIT_MENU") { const prompts = { edit_name: ["name", "Введіть нове ПІБ."], edit_need: ["need", "Введіть новий опис запиту."], edit_unit_number: ["militaryUnitNumber", "Введіть новий номер військової частини."], edit_city: ["city", "Введіть нове місто."], edit_np: ["novaPoshtaBranch", "Введіть нове відділення Нової пошти."] }; if (buttonId === "edit_recipient") { state.stage = "EDIT_RECIPIENT_CHOICE"; await saveUserState(from, state); await sendYesNoMenu(from, "Чи буде отримувачем інша особа?", "edit_recipient_other_yes", "edit_recipient_other_no"); return res.status(200).send("EVENT_RECEIVED"); } const selected = prompts[buttonId]; if (selected) { state.editField = selected[0]; state.stage = "EDIT_TEXT"; await saveUserState(from, state); await sendText(from, selected[1]); return res.status(200).send("EVENT_RECEIVED"); } await sendEditMenu(from, state.request.type); return res.status(200).send("EVENT_RECEIVED"); }
    ...
    if (state.stage === "CONFIRMATION") { if (buttonId === "confirm_request") { if (state.request.type === "individual") await saveApplicantProfile(from, state.request); if (!state.request.applicationNumber) state.request.applicationNumber = await createApplicationNumber(); if (!state.request.sheetRecordedAt) { await appendApplicationToSheet(state.request); state.request.sheetRecordedAt = new Date().toISOString(); } await saveUserState(from, state); await sendApplicationAccepted(from, state.request.applicationNumber); await resetUserState(from); } else if (buttonId === "edit_request") { state.stage = "EDIT_MENU"; await saveUserState(from, state); await sendEditMenu(from, state.request.type); } return res.status(200).send("EVENT_RECEIVED"); }
    if (state.stage === "OPERATOR") return res.status(200).send("EVENT_RECEIVED"); await sendMainMenu(from); return res.status(200).send("EVENT_RECEIVED");
  } catch (error) { console.error("Webhook error:", error); return res.status(200).send("EVENT_RECEIVED"); }
}
